...
export function TripMap({load,truckId,truckVersion,api,queue}:Props){
 const [showEvidence,setShowEvidence]=useState(false);const map=useRef<MapView|null>(null);const [mapReady,setMapReady]=useState(false);const [retry,setRetry]=useState(0);
 ...
 {unavailable?<Text style={{color:'#A23B30'}}>Map unavailable. Configure a restricted Android Maps key and rebuild the native app. Stop addresses remain below.</Text>:<View style={{height:280,borderRadius:10,overflow:'hidden'}}><MapView ref={map} style={{flex:1}} mapType="hybrid" initialRegion={{latitude:load.pickup.lat,longitude:load.pickup.lng,latitudeDelta:1.5,longitudeDelta:1.5}} onMapReady={()=>setMapReady(true)} accessibilityLabel={visible.stored?'Truck road route with pickup and delivery stops':'Pickup and delivery markers; truck route not available'}><Marker coordinate={{latitude:load.pickup.lat,longitude:load.pickup.lng}} title={load.pickup.name} description="Pickup"/><Marker coordinate={{latitude:load.delivery.lat,longitude:load.delivery.lng}} title={load.delivery.name} description="Delivery"/>{routeCoordinates.length>1?<Polyline coordinates={routeCoordinates} strokeColor="#E65C32" strokeWidth={5} geodesic={false}/>:null}</MapView></View>}
 <View style={{flexDirection:'row',alignItems:'center',gap:8}}>{visible.loading?<ActivityIndicator accessibilityLabel="Loading verified truck route"/>:null}<Text style={{flex:1,color:visible.stored?'#191B1D':'#666962'}}>{visible.message}</Text></View>
 {visible.stored?<><Text style={{fontWeight:'600',color:'#191B1D'}}>{Math.round(visible.stored.route.drivingMinutes)} min planned driving · {visible.cached?'Downloaded':'Synchronized'} route</Text><Pressable accessibilityRole="button" onPress={()=>setShowEvidence(!showEvidence)} style={{minHeight:44,justifyContent:'center'}}><Text style={{fontSize:13,fontWeight:'600',color:'#666962'}}>Route evidence {showEvidence?'−':'+'}</Text></Pressable>{showEvidence?<><Text style={{fontSize:12,color:'#666962'}}>Saved {new Date(visible.stored.savedAt).toLocaleString()} · {visible.stored.route.dataset}</Text><Text style={{fontSize:12,color:'#666962'}}>Truck {truckId}: {visible.stored.route.profile.height} m high · {visible.stored.route.profile.width} m wide · {visible.stored.route.profile.length} m long · {visible.stored.route.profile.weight} t gross · {visible.stored.route.profile.evidence}</Text><Text style={{fontSize:12,color:'#666962'}}>{visible.stored.route.warning}{visible.cached?' Current road conditions have not been refreshed. Base-map imagery may need a connection.':''}</Text></>:null}</>:<Text style={{fontSize:12,color:'#666962'}}>No route line is drawn without verified truck geometry.</Text>}
 <Pressable accessibilityRole="button" accessibilityState={{disabled:visible.loading||!request||!queue}} disabled={visible.loading||!request||!queue} onPress={()=>setRetry(x=>x+1)} style={{minHeight:44,padding:12,backgroundColor:'#EEEDE5',borderRadius:8,alignItems:'center',opacity:visible.loading?.5:1}}><Text style={{fontWeight:'600',color:'#191B1D'}}>Refresh truck route</Text></Pressable>
 </View>;
}
