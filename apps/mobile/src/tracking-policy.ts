export type TrackingGrant={id:string;scope:string;origin:string;userId:string;carrierId:string;driverId:string;sessionId:string;assignmentId:string;startedAt:string;expiresAt:string;enabled:boolean};
export type TrackingState={actor?:{role:string;driverId?:string;carrierId:string};workSessions?:{id:string;ended_at:string|null;started_at?:string}[];assignments:{id:string;driverId:string;loadId:string;status:string}[];loads:{id:string;provenance:string}[]};
export function trackingBlock(grant:TrackingGrant,state:TrackingState,now:number,permissionGranted:boolean):string|null{
 ...
 const assignment=state.assignments.find(a=>a.id===grant.assignmentId&&a.driverId===grant.driverId&&a.status==='accepted');
 if(!assignment)return 'Trip is no longer accepted by this driver.';
 if(!state.loads.some(l=>l.id===assignment.loadId&&l.provenance==='live'))return 'Only live trips can share device GPS.';
 return null;
}
