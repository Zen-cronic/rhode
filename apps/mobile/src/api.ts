...
export type State={capabilities?:{cachedDutyTelemetry?:boolean;emulatorTracking?:boolean};actor?:{role:string;driverId?:string;carrierId:string};serverTime:string;cursor:string;loads:Load[];assignments:Assignment[];resources:{id:string;kind:string;body?:any;version:number;duty?:'off_duty'|'on_duty'|'driving'|'sleeper'}[];proposals:{id:string;load_id:string;revision:number;status:string;body:any}[];visits:any[];invoices:any[];workSessions?:{id:string;version:number;status?:string;ended_at:string|null}[];stopCompletions?:{assignment_id:string;stop_id:string}[];dutyEvents?:any[];documents?:any[];facilityNotes?:FacilityNote[];planningRuns?:PlanningRun[];tripGroups?:TripGroup[];manifests?:Manifest[]};
