...
export type State={scenarios?:{id:string;clock:string}[];capabilities?:{cachedDutyTelemetry?:boolean;emulatorTracking?:boolean};actor?:{role:string;driverId?:string;carrierId:string};serverTime:string;cursor:string;loads:Load[];assignments:Assignment[];resources:{id:string;kind:string;body?:any;version:number;provenance?:string;budget?:import('../../../packages/domain/src/index').Driver['budget'];budgetAsOf?:string|null;hosEvidence?:(import('../../../packages/domain/src/hos-profile').HosProfileResult['hosEvidence'] & {revision?:number;sourceRef?:string;reviewedBy?:string});duty?:'off_duty'|'on_duty'|'driving'|'sleeper'}[];proposals:{id:string;load_id:string;revision:number;expected_version?:number;status:string;body:any}[];visits:any[];invoices:any[];workSessions?:(import("../../../packages/domain/src/tracking").MileageSession & {version:number;status?:string})[];stopCompletions?:{assignment_id:string;stop_id:string}[];dutyEvents?:any[];documents?:any[];facilityNotes?:FacilityNote[];planningRuns?:PlanningRun[];tripGroups?:TripGroup[];manifests?:Manifest[]};
