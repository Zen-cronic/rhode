  ...
  return <section className="panel tracking-panel">
    ...
    {!!assignmentId&&<>
      ...
      {!!samples.length&&<>
        ...
        {selected&&<div className="selected-sample" aria-live="polite"><h3>Selected sample · {when(selected.at)} ET</h3><p><strong>{dispositionLabel(selected.disposition)}</strong> · {selected.provenance} · {selected.duty.replaceAll('_',' ')}</p><p>Speed {sampleNumber(selected.speedKph,'km/h')} · Odometer {sampleNumber(selected.odometerKm,'km')} · Accuracy {sampleNumber(selected.accuracyM,'m')}</p><p className="fine">Sampled {when(selected.at)} ET · Received {when(selected.recordedAt)} ET</p>{selected.geofenceEvidence?.status==='ambiguous'&&<p role="status"><strong>Arrival held for review.</strong> {selected.geofenceEvidence.reason} Competing stops: {selected.geofenceEvidence.stopIds.join(', ')}. Source observation retained.</p>}{selected.geofenceEvidence?.stopStates?.length&&<div><h4>Fence confidence at this observation</h4>{selected.geofenceEvidence.stopStates.map(stop=><p className="fine" key={stop.stopId}>{stop.stopId} · {stop.confidence==='boundary'?'Boundary uncertainty — visit state held':stop.confidence==='inside'?'Confidently inside':stop.confidence==='outside'?'Confidently outside':'Uncertain'} · {stop.distanceM.toFixed(1)} m from center / {stop.radiusM} m fence</p>)}</div>}<code>{selected.id}</code>{validPosition(selected)&&<p className="fine">Recorded coordinate: {selected.position.lat}, {selected.position.lng}. Accuracy radius describes this sample only.</p>}<button onClick={()=>setSelectedId(null)}>Clear selected sample</button></div>}
        ...
        <ol className="tracking-samples">{[...samples].reverse().map(point=><li key={point.id} className={`tracking-sample disposition-${point.disposition}`}><div className="panel-heading"><button aria-pressed={selectedId===point.id} onClick={()=>setSelectedId(point.id)}>{when(point.at)} ET</button><span className={`tag ${point.disposition==='applied'?'status-accepted':'status-pending'}`}>{dispositionLabel(point.disposition)}</span></div><dl><div><dt>Speed</dt><dd>{sampleNumber(point.speedKph,'km/h')}</dd></div><div><dt>Odometer</dt><dd>{sampleNumber(point.odometerKm,'km')}</dd></div><div><dt>GPS accuracy</dt><dd>{sampleNumber(point.accuracyM,'m')}</dd></div></dl><p className="fine">{point.provenance} · {point.duty.replaceAll('_',' ')}</p>{point.geofenceEvidence?.status==='ambiguous'&&<p className="fine"><strong>Arrival held for review</strong> · {point.geofenceEvidence.stopIds.join(', ')}</p>}<code>{point.id}</code></li>)}</ol>
