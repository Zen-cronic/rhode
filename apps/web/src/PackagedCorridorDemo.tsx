...
export function PackagedCorridorDemo({session,online}:{session:Session;online:boolean}){
 ...
 return <details className="panel simulator-controls packaged-corridor-demo" onToggle={event=>setMounted(event.currentTarget.open)}><summary>Judge replay · matched Highway 401 slowdown</summary>{mounted&&<div className="simulator-content"><p className="fine packaged-replay-intro">Two complete API-acknowledged synthetic recordings are packaged with this preview. The replay is read-only: its controls move only the historical view cursor and issue no operational command.</p><CorridorReplay run={baseline} matchedRun={slowdown} session={session} online={online} recordingLoader={loader} initialOpen initialCompare/></div>}</details>;
}
