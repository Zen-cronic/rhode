...
Status: accepted locally and on the public synthetic preview on September 13, 2026.
...
At the shared final cursor, the baseline has traveled 40.973 km and the slowdown branch 13.316 km. The 3D view renders the baseline vehicle in ivory, the slowdown vehicle in signal orange and a restrained connector labeled as the recorded progress gap. A progressive paired profile adds up to 64 exact source pairs: ivory cylinders for baseline and signal-orange fins for slowdown, with height equal to recorded km/h. Missing speeds, route-epoch mismatches and unmatched observations remain gaps. The displayed gap is 27.657 km. Deterministic simulator forecasts place the slowdown completion 1,606 seconds, or 26m46s, after the baseline. Speed, odometer and completion values come from the two retained recordings; the browser does not recalculate vehicle movement or interpolate positions.
...
- [`verification.json`](evidence/3d-slowdown-comparison-2026-09-12/verification.json) records the 64-pair profile contract, its 128-instance/two-draw-call ceiling, and the shared server-derived basis, distinct conditions hashes, both 2,401-event branches, the 27.657 km progress gap, the 1,606-second finish delta, zero browser writes and zero page errors.
...
The public viewer at `/?view=matched-401` was verified against web revision `roadstar-web-00074-cic`: all 64 pairs, the toggle, diagram fallback, narrow layout and offline inspection passed with zero operational API calls or writes.
