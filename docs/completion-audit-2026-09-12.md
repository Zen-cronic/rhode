...
| Maps and 3D explanation | Google Maps road/satellite mode remains the operational map. Lazy Three.js views project retained simulator events with source IDs, shared clocks, diagram/reduced-motion fallbacks and no authority to approve or mutate operations. The corridor profile maps recorded km/h to height, preserves at-or-before source identity and leaves missing/mismatched evidence as gaps. |
...
| `npm run build --workspace @roadstar/web` | **Pass, 312 modules transformed.** Three.js remains lazy for the two 3D surfaces; Vite reports the known large-chunk advisory. |
...
| `node scripts/verify-deployed-release.mjs` | **Pass.** Optimizer `00015-vuk`, API `00036-woy`, web `00076-piv` and documents `00021-tam` are ready at 100% traffic. Hosted MP4/SRT/PDF/PPTX/replay bytes match tracked masters; ranked replacement is accepted. |
| Hosted public matched replay verifier | **Pass.** All 64 exact speed pairs render at the intervention end; the keyboard toggle, diagram fallback, 390 px layout and offline-after-load inspection pass with 0 operational API requests, 0 writes and 0 page errors. |
| Hosted public dock browser verifier | **Pass.** Five milestones, exact HOS headroom and 144-minute requirement, London/Milton scenario separation, editable value recalculation, 3D/diagram, 390 px and offline-after-load inspection; 0 API requests, 0 writes, 0 page errors. |
...
| Film audio signal quality | **Pass.** The complete AAC-LC mono48kHz stream matches the container duration and measures−16.0LUFS integrated,2.5LU range and−1.4dBFS true peak. No silence interval of0.8seconds or longer appears below−45dBFS. Film, caption and audio-packet hashes remain pinned in their separate receipts. This does not replace the required human listen for pronunciation, pacing and meaning. |
| Caption readability | **Pass structurally and in five sampled film frames.** The narrated text is preserved byte-independently across 54 source cues and 77 formatted cues; each cue uses at most two 42-character lines, reading speed peaks at 21.999 visible characters per second and the hosted SRT matches the tracked master. Human end-to-end caption playback remains required. |
| Staged judge identity readiness | **Pass.** Three storage-empty hosted Chrome contexts authenticate the dispatcher and both drivers, load the staged carrier, expose only their role-specific navigation at desktop/390px and return to login after sign-out. The verifier records zero operational POSTs/errors and retains no credentials, token, browser state or screenshot. Credential delivery and repository invitations remain operator gates. |
