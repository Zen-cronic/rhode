...
`node scripts/assemble-current-demo.mjs` builds a candidate under `/tmp/roadstar-demo-current/` without overwriting the accepted public asset. The prior film is preserved under the local submission package before promotion. `node scripts/verify-current-demo.mjs` confirms the web asset and submission package are byte-identical, the original narration stream is unchanged, the four-minute H.264/AAC 1920×1080 24fps contract remains intact, and the complete file decodes without errors.

Evidence and inspected representative frames are in `docs/evidence/demo-current-2026-09-12/`. Human listening, live-stage rehearsal and acceptance of the direct MP4 URL by the final submission form remain operator checks.
