...
function slide(dark=false){const s=p.slides.add();s.background.fill=dark?C.graphite:C.ivory;const i=p.slides.items.length;const source=(noteStrings[i-1]||'').replace(/^\d+ seconds\. /,'').replace('21 backend','22 backend');s.speakerNotes.textFrame.setText(`${durations[i-1]} seconds. ${source}\n\nVisual source: actual RoadStar application captures where shown. Generated road sculpture is illustrative brand artwork, not geographic or route evidence. Design system: docs/design/precision-transport.md. Current check counts: 22 backend, 23 native mobile. Core slides 1–11 total 10 minutes. Appendix slides 12–13 total 5 minutes.`);txt(s,String(i).padStart(2,'0'),1183,661,46,28,17,false,dark?C.light:C.muted);return s;}
