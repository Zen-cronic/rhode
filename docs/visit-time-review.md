...
Local packet fixture carrier: `dock-document-c50e2b62-4ded-4a34-a2ea-34e4c5eb0a94`. Local preview http://localhost:5174 is current. At that local acceptance, migrations018/019 remained local; hosted API00020-mxh/web00033-84x/documents00010-d4d/optimizer00007-lhb are unchanged. Native correction UI and corrected-time approval parity remain next; older clients cannot approve corrected invoices without explicit corrected-time acknowledgement. Physical Android/iOS remain deferred.
...
## Hosted verification — September 11, 21:49 UTC

API `roadstar-api-00021-qwk`, web `roadstar-web-00034-nv7` and document worker `roadstar-documents-00011-rhf` serve 100% traffic. Optimizer `roadstar-optimizer-00007-lhb` is unchanged. Existing seed execution `roadstar-seed-4hs4c` completed and its structured output confirmed migration018's visit-session column and migration019's correction table. Image digests and traffic receipts are in `docs/evidence/cloud-dock-evidence-2026-09-11/deployment.json`.

The separate synthetic carrier `dock-evidence-cloud-20260911` completed Firebase sign-in, dispatch/driver acceptance, telemetry ingestion, the original $75 invoice approval, authenticated PDF upload to Cloud Storage, existing Scheduler/Tasks/Vertex extraction, exact-byte source download, stale document-version rejection, reviewed time correction, exact command replay, and separate $50 corrected invoice approval through the deployed web. Four invoice revisions and one correction remain. API-visible tracking observations, original visits and resources were unchanged. This hosted check did not independently compare raw database rows; that invariant is covered by the earlier PostgreSQL tests and local built-web proof.

The driver cannot read dispatcher correction review; an unassigned driver cannot download the source; the same dispatcher in another carrier receives 404 for the source. Six hosted captures were inspected, including desktop, 390px and offline states. No page errors or horizontal overflow. The approval dialog scrolls to its action. Receipt verification.json includes the actual source hash, extracted fields, actors, correction and invoice IDs.

`scripts/verify-cloud-visit-time-review.mjs` uses ignored preview credentials and public Firebase configuration. It is a mutating acceptance harness for a separately seeded, initially empty carrier. The `/tmp/roadstar-cloud-dock-fixture.json` receipt permits continuation before correction; a completed correction fails closed on rerun before any additional source review. It must not be pointed at real customer operations. No credentials are retained in evidence. The initial attempt ran before the seed job completed and received403 without mutation; the same live seed execution was observed to completion, then verification passed.

One synthetic document was submitted to the existing enabled Gemini2.5Flash extraction worker under the previously approved preview allowance. No new resource was provisioned. Actual billed cost and token usage were not measured in this packet. This is extraction evidence, not model authority to choose billing times. Native billing/correction parity and physical/iOS verification remain outstanding; the third design pass and current Android preview were unchanged. Main remains the private GitHub default branch.
