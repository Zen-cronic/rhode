...
test('HOS review preserves CSV, retries once, invalidates proposals and requires dispatcher ownership',async()=>{
 const {dispatcher,driver,simulator,carrierId}=await setup();const other=await setup();const proposed:any=await store.propose(dispatcher,command(),input);
 const raw={driverId:'D-01',cycle:1 as const,dayAnchor:'2026-09-13T04:00:00Z',sourceName:'reviewed-duty.csv',sourceCsv:`start,end,duty\n2026-08-28T04:00:00Z,${DEMO_NOW},off_duty\n`,reason:'Reviewed complete synthetic ledger for this scenario',acknowledgePlanningLimits:true as const,acceptIncomplete:false};
 const before=(await store.snapshot(dispatcher)).cursor,preview=await store.previewHos(dispatcher,raw);assert.equal(preview.expectedVersion,1);assert.equal(preview.budget?.drivingMinutes,780);assert.equal((await store.snapshot(dispatcher)).cursor,before);assert.equal((await db.query('SELECT count(*)::int AS n FROM hos_profiles WHERE carrier_id=$1',[carrierId])).rows[0].n,0);
 for(const actor of [driver,simulator]){await assert.rejects(store.previewHos(actor,raw),{code:'FORBIDDEN'});await assert.rejects(store.reviewHos(actor,command(),raw),{code:'FORBIDDEN'});}
 await assert.rejects(store.reviewHos({...dispatcher,carrierId:other.carrierId},command(),{...raw,driverId:'missing'}),{code:'NOT_FOUND'});
 const cmd=command(preview.expectedVersion),reviewed:any=await store.reviewHos(dispatcher,cmd,raw);assert.equal(reviewed.version,2);assert.equal(reviewed.revision,1);assert.deepEqual(await store.reviewHos(dispatcher,cmd,raw),reviewed);assert.equal((await db.query('SELECT count(*)::int AS n FROM hos_profiles WHERE carrier_id=$1',[carrierId])).rows[0].n,1);const source=(await db.query('SELECT source_text,source_sha256,review_reason FROM hos_profiles WHERE carrier_id=$1',[carrierId])).rows[0];assert.equal(source.source_text,raw.sourceCsv);assert.equal(source.source_sha256,preview.sourceSha256);assert.equal(source.review_reason,raw.reason);assert.equal((await store.hosHistory(driver,'D-01')).record.source_text,raw.sourceCsv);await assert.rejects(store.hosHistory(await store.membership('demo-driver-2',carrierId),'D-01'),{code:'FORBIDDEN'});assert.equal((await store.hosHistory(other.dispatcher,'D-01')).record,null);
 await assert.rejects(store.reviewHos(dispatcher,cmd,{...raw,reason:'Changed source explanation'}),{code:'KEY_REUSED'});await assert.rejects(store.reviewHos(dispatcher,command(1),raw),{code:'STALE_VERSION'});await assert.rejects(store.approve(dispatcher,command(),{proposalId:proposed.id}),{code:'STALE_PROPOSAL'});
 const short={...raw,sourceCsv:`start,end,duty\n2026-09-13T04:00:00Z,${DEMO_NOW},off_duty\n`};assert.equal((await store.previewHos(dispatcher,short)).budget,null);await assert.rejects(store.reviewHos(dispatcher,command(2),short),{code:'INCOMPLETE_HISTORY'});await store.reviewHos(dispatcher,command(2),{...short,acceptIncomplete:true});assert.equal((await store.snapshot(driver)).resources.find(r=>r.id==='D-01')!.budget,null);
 const app=createApi(store,{localDemo:true});const headers={authorization:'Bearer demo-dispatcher','x-carrier-id':carrierId};const response=await app.inject({method:'POST',url:'/api/hos-preview',headers,payload:raw});assert.equal(response.statusCode,200);assert.equal(response.headers['cache-control'],'private, no-store');await app.close();
});
