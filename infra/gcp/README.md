...
September 13 verified revisions: API `00036-woy`, optimizer `00015-vuk`, web `00076-piv` and documents `00021-tam` serve 100% traffic. The current hosted matched replay adds a 64-pair exact-observation speed profile to the demand-rendered 3D/diagram views. The public dock receipt now exposes exact retained D-01 HOS headroom and clearly separates the London receipt from the film's Milton example. The caption sidecar still matches the 77-cue readable master; film bytes and all other services are unchanged. Migration020, source-bound axle review, planned-wait duty accounting and native-to-web manifest acceptance are verified in [cloud-axle-review.md](../../docs/cloud-axle-review.md). The current four-minute film includes the bounded organizer-value scenario and is byte-verified against the hosted artifact. Inspect current revisions before redeploying because later checkpoints may advance these values.
