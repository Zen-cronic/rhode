...
export class Store {
  ...
  async check(c:pg.PoolClient,a:Actor,load:Load,input:Row,ignoreId?:string){
    ...
    const axle=await axleProof(c,a.carrierId,load,truck,trailer);result.reasons.push(...axle.reasons);Object.assign(result,{axle});
  ...
  async axleReview(a:Actor,loadId:string,truckId:string,trailerId:string){const c=await this.db.connect();try{await c.query('BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY');await this.authorizeLoad(c,a,loadId);if(a.role==='driver')demand((await c.query("SELECT 1 FROM assignments WHERE carrier_id=$1 AND load_id=$2 AND driver_id=$3 AND truck_id=$4 AND trailer_id=$5 AND status IN ('offered','accepted','completed')",[a.carrierId,loadId,a.driverId,truckId,trailerId])).rows.length,'FORBIDDEN','Assigned vehicle required.',403);const load=await this.load(c,a,loadId),truck=await this.resource<Truck>(c,a,truckId,'truck'),trailer=await this.resource<Trailer>(c,a,trailerId,'trailer');const history=(await c.query('SELECT * FROM axle_assessments WHERE carrier_id=$1 AND load_id=$2 AND truck_id=$3 AND trailer_id=$4 ORDER BY revision DESC LIMIT 100',[a.carrierId,loadId,truckId,trailerId])).rows;const proof=await axleProof(c,a.carrierId,load,truck,trailer);await c.query('COMMIT');return {loadId,truckId,trailerId,history,proof};}catch(error){await c.query('ROLLBACK');throw error;}finally{c.release();}}
  reviewAxles(a:Actor,cmd:Command,input:import('../../../packages/domain/src/axle.ts').AxleReviewInput){return reviewAxles(this,a,cmd,input);}
  ...
  optimize(a:Actor,cmd:Command,input:Row){this.dispatcher(a);return this.command(a,cmd,'planning.computed',input,async c=>{
    ...
    for(const candidate of input.vehicles){let driver=await this.resource<Driver>(c,a,candidate.driverId,'driver');const truck=await this.resource<Truck>(c,a,candidate.truckId,'truck'),trailer=await this.resource<Trailer>(c,a,candidate.trailerId,'trailer');
      if(truck.axleClearance!=='verified'){rejected.push({vehicle_id:truck.id,reason:'Truck and axle clearance is unverified.'});continue;}
    ...
    const axleRejected:Row[]=[];
    for(const vehicle of vehicles){const truck=await this.resource<Truck>(c,a,vehicle.truckId,'truck'),trailer=await this.resource<Trailer>(c,a,vehicle.trailerId,'trailer');for(const shipment of shipments){const proof=await axleProof(c,a.carrierId,loads.find(l=>l.id===shipment.id)!,truck,trailer);shipment.allowed_vehicles??=[];shipment.axle_demands_g??={};if(proof.eligible){shipment.allowed_vehicles.push(vehicle.id);if(proof.required&&proof.demandsG){shipment.axle_demands_g[vehicle.id]=proof.demandsG;vehicle.axle_capacity_g=proof.capacitiesG;vehicle.capacity_lb=Math.min(vehicle.capacity_lb,proof.grossPayloadLb!);}}else axleRejected.push({load_id:shipment.id,vehicle_id:vehicle.id,reason:proof.reasons.join(' ')});}}
    ...
    const result=vehicles.length&&shipments.length?await computation('/optimize-roads',request):{status:'no_eligible_inputs',routes:[],infeasible_loads:[]};result.candidate_rejections=axleRejected;for(const shipment of shipments){if(!vehicles.length)result.infeasible_loads.push({load_id:shipment.id,reason:'No eligible vehicle pairing.'});else if(!shipment.allowed_vehicles.length){const entry=result.infeasible_loads.find((item:any)=>item.load_id===shipment.id);if(entry)entry.reason=axleRejected.filter(item=>item.load_id===shipment.id).map(item=>`${item.vehicle_id}: ${item.reason}`).join(' ');}}result.infeasible_loads.push(...rejected);result.assumptions=[...(result.assumptions??[]),'Pickup appointment fixed at supplied start; delivery must finish by supplied end','26-pallet synthetic trailer allowance; verify before live use','Optimization is a proposal; route execution still requires dispatcher approval',...(vehicles.some(v=>!v.axle_capacity_g)?['Unassessed synthetic pairings use declared axle-clearance flags; physical axle loading remains unverified.']:[])];
  ...
  async seed(carrierId='demo-carrier'){
    const c=await this.db.connect();try{await c.query('BEGIN');await c.query('SELECT pg_advisory_xact_lock(hashtext($1))',[carrierId]);
      ...
      await c.query('INSERT INTO carriers VALUES($1,$2)',[carrierId,'Rhode · synthetic demonstration']);const data=fixtures();
