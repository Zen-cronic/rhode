import {evaluateHosProfile,type HosProfile} from '../../../packages/domain/src/hos-profile.ts';
import type pg from 'pg';
import type {Driver} from '../../../packages/domain/src/index.ts';
import {consumeDutyHistory} from '../../../packages/domain/src/hos.ts';
export async function withDutyHistory(db:Pick<pg.Pool,'query'>|pg.PoolClient,carrierId:string,drivers:Driver[],override?:{driverId:string;body:HosProfile}){
 ...
 ) e WHERE (p.body IS NOT NULL OR r.id=$3 OR e.at>=b.at)) AS observations
 ...
 WHERE r.carrier_id=$1 AND r.id=ANY($2::text[])`,[carrierId,drivers.map(d=>d.id),override?.driverId??null])).rows;
 return drivers.map(driver=>{const row=records.find(r=>r.id===driver.id);const basis=row?.at&&row.provenance===driver.provenance?{at:new Date(row.at).toISOString(),duty:row.duty,budget:row.budget}:null;const at=row?.clock?new Date(row.clock).toISOString():new Date().toISOString();const preview=override?.driverId===driver.id;const derived=preview?evaluateHosProfile(override!.body,row?.observations??[],at):row?.profile_revision?evaluateHosProfile(row.profile_provenance===driver.provenance?row.profile:null,row?.observations??[],at):consumeDutyHistory(basis,row?.observations??[],at);return {...driver,...derived,hosEvidence:{...derived.hosEvidence,...(!preview&&row?.profile_revision?{revision:row.profile_revision,sourceRef:row.profile_source,reviewedBy:row.profile_reviewer}:{})}};});
}
