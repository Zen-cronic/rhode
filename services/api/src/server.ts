...
export function createApi(store:Store,options:{localDemo?:boolean;verifyToken?:(token:string)=>Promise<string>}={}){
  ...
  for(const [route,schema] of Object.entries(schemas))app.post(`/api/${route}`,async(req)=>{
    ...
    switch(route){case 'simulation-clock':return store.advanceSimulationClock(a,cmd,b);case 'approve-plan':return store.approvePlan(a,cmd,b);case 'optimize':return store.optimize(a,cmd,b);case 'review-document':return store.reviewDocument(a,cmd,b);case 'approve-invoice':return store.approveInvoice(a,cmd,b);case 'facility-note':return store.facilityNote(a,cmd,b);case 'maintenance':return store.maintenance(a,cmd,b);case 'push-token':return store.pushToken(a,cmd,b);case 'document':return store.document(a,cmd,b);case 'delay':return store.delay(a,cmd,b);case 'duty':return store.duty(a,cmd,b);case 'complete-stop':return store.completeStop(a,cmd,b);case 'dispatch':return store.dispatch(a,cmd,b);case 'propose':return store.propose(a,cmd,b);case 'approve':return store.approve(a,cmd,b);case 'respond':return store.respond(a,cmd,b);case 'work-session':return store.workSession(a,cmd,b);case 'telemetry':return store.ingest(a,cmd,b);case 'bind-contract':return store.bindContract(a,cmd,b);case 'detention':return store.detentionDraft(a,cmd,b);}
  });
