...
export async function roadRoute(c:pg.PoolClient,carrierId:string,load:Load,truck:Truck,origin?:Point){
 ...
 const closures=(await c.query("SELECT r.id,r.area FROM road_closures r JOIN assignments a ON a.carrier_id=r.carrier_id AND a.id=r.assignment_id WHERE r.carrier_id=$1 AND a.load_id=$2 AND a.status IN ('offered','accepted') ORDER BY r.id",[carrierId,load.id])).rows.map(r=>({id:r.id,...r.area}));
 const locations=[...(origin?[origin]:[]),load.pickup,load.delivery].map(p=>({lat:p.lat,lon:p.lng}));
 const fingerprint=createHash('sha256').update(JSON.stringify({locations,profile,closures,loadId:load.id,truckId:truck.id,dataset:process.env.ROUTING_DATASET})).digest('hex');
 ...
 const result=await computation('/route',{locations,truck:profile,closures});demand(result.routing_evidence==='valhalla-truck'&&result.route?.trip?.legs?.length===locations.length-1,'ROUTING_INVALID','Routing service returned incomplete route evidence.',503);
