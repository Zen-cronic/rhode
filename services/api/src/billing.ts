...
export async function prepareDetention(c:pg.PoolClient,carrierId:string,visit:any,options:{automatic:boolean;expectedVersion?:number;contractId?:string}){
 ...
 if(options.automatic&&last>0)return null;
 const correction=await latestVisitTime(c,carrierId,visit.id);await validateVisitTime(c,carrierId,visit,correction);
 const timing=correction?.body,arrivalAt=timing?.arrivalAt??visit.arrival,departureAt=timing?.departureAt??visit.departure;
 const dwellMinutes=Math.floor((new Date(departureAt).getTime()-new Date(arrivalAt).getTime())/60000),billableMinutes=Math.max(0,dwellMinutes-contract.free_minutes);

 if(options.automatic&&billableMinutes===0)return null;
 if(!options.automatic)demand(last===options.expectedVersion,'STALE_VERSION','Invoice revision changed.');
 const timingConflicts=await visitTimeConflicts(c,carrierId,visit,correction);
 const body={...(timingConflicts.length?{timingConflicts}:{}),visitPolicy:visitPolicyEvidence,dwellMinutes,billableMinutes,amountCents:Math.round(billableMinutes*contract.rate_cents_per_hour/60),currency:contract.currency,evidence:[visit.arrival_event,visit.departure_event],contract,shipmentTerms:{loadId:visit.load_id,mode:load.mode,boundBy:bound_by,boundAt:bound_at},rounding:'whole-minute floor, nearest cent',...(correction?{timingEvidence:{...timing,correctionId:correction.id,revision:correction.revision}}:{}),precision:correction?'reviewed_document_times':'observed_samples',requiresEvidenceReview:true,automatic:options.automatic};
