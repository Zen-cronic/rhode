...
const expected={optimizer:'roadstar-optimizer-00015-vuk',api:'roadstar-api-00036-woy',web:'roadstar-web-00076-piv',documents:'roadstar-documents-00021-tam'};
const services={};
...
const webResponse=await fetch(web);assert.equal(webResponse.status,200);assert.match(await webResponse.text(),/Rhode/);
const publicReplayResponse=await fetch(web+'/?view=matched-401');assert.equal(publicReplayResponse.status,200);assert.match(await publicReplayResponse.text(),/Rhode/);
const publicDockResponse=await fetch(web+'/?view=dock-evidence');assert.equal(publicDockResponse.status,200);assert.match(await publicDockResponse.text(),/Rhode/);
