...
try{
 ...
 try{for(let tick=0;tick<cycles;tick++){
  ...
  await Promise.all(ids.map(async({uid})=>{const t=performance.now(),headers={Authorization:`Bearer ${uid}`,'X-Carrier-Id':carrierId};const updates=await fetch(base+`/api/updates?cursor=${cursor}`,{headers});const changes=await updates.json() as any;if(!updates.ok)failures.push(JSON.stringify(changes));const state=await fetch(base+'/api/state',{headers});const snapshot=await state.json() as any;if(!state.ok||snapshot.assignments?.length!==1)failures.push('Invalid driver snapshot');syncMs.push(performance.now()-t);lagMs.push(performance.now()-committedAt.get(uid)!);}));
